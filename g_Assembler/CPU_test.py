...
def test_CPU():
    ...
    file_test = open("../f_Assembly/config_testes_nasm.txt","r")
            ...
            if ((line.strip()[0] != '#') and ( line.strip().find('.nasm') > 0 )):
                ...
                if os.path.isfile(mif):
                    ...
                    for test in range(0, int(par[1])):
                        ramIn = "../f_Assembly/tests/" + name + "/" + name +"{}".format(test)+ "_in.mif"
                        ramTest = "../f_Assembly/tests/" + name + "/" + name +"{}".format(test)+ "_tst.mif"
